...
<div class="container mt-5">
    <h1 class="text-center mb-4" style="font-weight: bold; color: #007bff;">Daftar Pendapatan</h1>
    ...
    @endif
    <div class="mb-3">
        <form method="GET" action="{{ route('pendapatan.index') }}" class="row g-2">
            ...
            <div class="col-12 col-md-2">
                <a href="{{ route('pendapatan.index') }}" class="btn btn-danger w-100">Clear Filter</a>
            </div>
        </form>
    </div>

    <div class="mb-3 d-flex justify-content-between">
        <a href="{{ route('pendapatan.create') }}" class="btn btn-primary"> + Pendapatan</a>
        <button id="exportBtn" class="btn btn-success">Ekspor Excel</button>
        {{-- <a href="{{ route('pendapatan.export') }}" class="btn btn-success">Export Excel</a> --}}
    </div>
    ...
    <table class="table table-striped table-bordered" id="dataTable">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $pendapatan->kategori->nama_kategori }}</td>
                <td>{{ number_format($pendapatan->jumlah, 2, ',', '.') }}</td>
                <td>{{ \Carbon\Carbon::parse($pendapatan->tanggal)->format('d-m-Y') }}</td>
                <td>{{ $pendapatan->deskripsi }}</td>
                <td style="text-align: center;">
                    <a href="{{ route('pendapatan.edit', $pendapatan->id) }}" class="btn btn-link text-primary p-0" title="Edit">
                        <i class="bi bi-pencil-square"></i> <!-- Ikon Edit -->
                    </a>
                    <form action="{{ route('pendapatan.destroy', $pendapatan) }}" method="POST" style="display: inline-block;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-link text-danger p-0" title="Hapus" onclick="return confirm('Apakah Anda yakin ingin menghapus pendapatan ini?');">
